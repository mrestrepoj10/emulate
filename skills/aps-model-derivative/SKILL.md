...
- In the local APS emulator, jobs accept `svf2`, `svf`, and `thumbnail`; manifests advance lazily without timers, `x-ads-force: true` resets the job, and terminal observation emits `extraction.finished` once. Thumbnail and metadata reads return `202` while translating. Successful jobs expose a deterministic placeholder PNG, stable view GUIDs, a synthetic object tree, and small deterministic property bags. Manifest deletion requires `data:write`. The emulator never serves geometry or a full property database.
