...
Autodesk Platform Services (formerly Forge) emulation with authentication v2, Data Management reads and uploads, simulated Model Derivative translation, Autodesk Construction Cloud Issues, RFIs, Sheets, and Model Coordination reads, plus active Webhooks delivery and local event simulation.
...
Folder contents returns mixed folders and items with their tip versions in `included`. It supports `filter[type]`, `filter[extension.type]`, and zero-based `page[number]` with `page[limit]` up to 200. Version history is newest first.
