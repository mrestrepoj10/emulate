...
Autodesk Platform Services (formerly Forge) emulation with authentication v2, Data Management, Model Derivative, Autodesk Construction Cloud Issues, RFIs, Sheets, and Model Coordination reads, plus active Webhooks delivery and local event simulation.
...
With no config, the emulator seeds a confidential client `aps-test-client` / `aps-test-secret`, a public client `aps-test-app`, a user `testuser@autodesk.local`, one hub, two projects, one ACC project membership, sample Issues, RFIs, Sheets, two coordinated Docs models with manifests, and one successful clash test.
...
ACC resources use the Data Management project ID in seed config. Issues, RFIs, and Model Coordination use that ID without `b.` in request paths. Sheets accepts either form.
