...
Autodesk Platform Services (formerly Forge) emulation with authentication v2, Data Management reads, recursive search, uploads and downloads, simulated Model Derivative translation and inspection, Autodesk Construction Cloud Issues, RFIs, Sheets, and Model Coordination reads, plus active Webhooks delivery and local event simulation.
...
- `GET /bim360/modelset/v3/containers/:containerId/modelsets` — list model sets
- `GET /bim360/modelset/v3/containers/:containerId/modelsets/:modelSetId/versions/latest` — get the latest coordinated document versions
...
Folder contents returns mixed folders and items with their tip versions in `included`. It supports `filter[type]`, `filter[extension.type]`, and zero-based `page[number]` with `page[limit]` up to 200. Recursive search starts at one project folder, returns items only with tips in `included`, and accepts case-insensitive `filter[attributes.displayName]` contains matching plus comma-separated `filter[fileType]`. Version history is newest first.
...
The supported upload path is project storage followed by `signeds3upload`, signed part PUTs, upload completion, and then item or version creation. Finalized objects support `signeds3download`; its signed GET URL returns the exact stored bytes and recorded content length. Direct deprecated OSS object PUTs, multipart resume and abort, batch downloads, and geometry are intentionally outside this emulator surface.

Formats, manifests, thumbnails, and metadata also accept a 2-legged `data:read` token:
...
METADATA=$(curl -s "$APS_EMULATOR_URL/modelderivative/v2/designdata/$SAMPLE_URN/metadata" \
  -H "Authorization: Bearer <2-legged-access-token>")
VIEW_GUID=$(printf '%s' "$METADATA" | jq -r '.data.metadata[0].guid')
curl "$APS_EMULATOR_URL/modelderivative/v2/designdata/$SAMPLE_URN/metadata/$VIEW_GUID/properties" \
  -H "Authorization: Bearer <2-legged-access-token>"
```

Thumbnail and metadata reads return `202` while translation is active. Successful thumbnails are a deterministic placeholder PNG. Metadata exposes stable synthetic view GUIDs, object trees, and small property bags for inspection tests. It is not source geometry or a complete property database, and the emulator still serves no geometry. `DELETE /modelderivative/v2/designdata/:urn/manifest` requires `data:write`; a new job for an uploaded source restores the manifest.
...
Use a 3-legged `data:read` token and the project ID without `b.`. The seeded model set references the emulator's shared Data Management document versions and Model Derivative manifests.
...
curl -X POST "$APS_EMULATOR_URL/_aps/simulate/modelset-version-added" \
  -H "Content-Type: application/json" \
  -d '{"modelSetId":"13131313-1313-4131-8131-131313131313"}'
```

Lists use opaque continuation tokens and accept at most 20 items per page. Signed resource URLs expire; call the resources endpoint again to issue fresh URLs. The simulator moves the new test through `Pending`, `Processing`, and `Success` on the configurable compressed clock.
