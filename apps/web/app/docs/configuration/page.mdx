...
Every APS project `hub_id` must match a hub in the effective seed data.
Folders reference parents, items reference folders, and versions reference items. Invalid parents and folder cycles fail during seeding.
ACC resources use the Data Management project ID in seed config. Remove `b.` for Issues and RFIs request paths. Sheets accepts either form.
`upload.maxObjectBytes` caps in-memory signed uploads. Translation duration is evaluated lazily on manifest reads, and `failForExtensions` values may be written with or without a leading dot.
